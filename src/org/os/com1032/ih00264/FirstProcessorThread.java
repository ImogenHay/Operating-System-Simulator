 ...
 * @author ih00264
 */
public class FirstProcessorThread extends Thread {
	String frequency;
	ReentrantLock lock;
	Condition con;
	...
	public FirstProcessorThread (String frequency, ReentrantLock lock, Condition con, Vector<PCB> running_buffer, MemoryManagementUnit mmu, Instructions decodeExecute) {
		 ...
		 while(true) {
				...
				while(this.running_buffer.isEmpty()) {
					try {
						...
						System.out.println("[CPU1] Can't await");
						e.printStackTrace();
					} 
				}
		 }
	...
	public void processing (PCB program) { //fetch decode execute cycle
		System.out.println("[CPU1] Fetching Process " + program.getID() + "s program\n");
		...
		if (instructionCount > 0) { //if process has program
			System.out.println("[CPU1] Decoding Process " + program.getID() + "s program\n");
			...
			System.out.println("--------------------------------------------------------------------------------\n");
			program.executed();
			System.out.println("[CPU1] Executed Process " + program.getID() + "s program\n");
			this.burstTime+= instructionCount; //CPU burst is count of instructions
			this.updateClock(instructionCount);
			System.out.println("[CPU1] Clock = " + this.clock + "\n");
		}
	}
}
